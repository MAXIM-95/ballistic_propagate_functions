...
class CPUMPSKPropagator: public CPUBasePropagator{
private:
  T                   _centerx,
                      _centery,
  ...
  CPUMPSKPropagator(const CPUMPSKPropagator&);
  CPUMPSKPropagator &operator=(const CPUMPSKPropagator&);
...
public:
  /*!
   * \brief Cоздание и удаление калькулятора траекторий в МПСК
   ...
   */
  CPUMPSKPropagator(const T& ib, const T& ih,
  ...
  inline virtual ~CPUMPSKPropagator(){}
