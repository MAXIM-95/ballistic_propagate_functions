...
#include "cpuMSIGMballisticspgsk.h"
...
CPUPGSKPropagator::CPUPGSKPropagator(IStaticAtmosphere::ISATypes atm_type,
...
T CPUPGSKPropagator::heightInBaseSystem(const T &x, const T &y, const T &z)
{
...
//--------------------------------------------------------------
bool CPUPGSKPropagator::accelerations(const T &x, const T &y, const T &z,
                                      ...
                                      const T &atmorange)
{
  ...
  dvx = k0*center[0] + 2.*_datum.AngularVelocity()*vy;
  dvy = k0*center[1] - 2.*_datum.AngularVelocity()*vx;
  dvz = k0*center[2] - k1;


  //Расчет высоты объекта
  T height = heightInPGSK(center[0], center[1], center[2], _datum);
#ifdef G_ASSERT_SHEME
...
T CPUPGSKPropagator::altitude_rate(const T &x,  const T &y,  const T &z,
                                   const T &vx, const T &vy, const T &vz)
{
  //Вектор дальности от объекта к центру Земли в ПГСК
  T  center_x = x,
...
CPUBasePropagator::PropagateResult
                   CPUPGSKPropagator::propagate(T &t, const T &dt,
                                                T &x, T &y, T &z,
                                                T &vx, T &vy, T &vz,
                                                T &dvx, T &dvy, T &dvz,
                                                const T &gamma, const T &gamma1,
...
CPUBasePropagator::PropagateResult
     CPUPGSKPropagator::propagateOnHeight(T &t, const T &dt,
                                          T &x, T &y, T &z,
                                          T &vx, T &vy, T &vz,
                                          T &dvx, T &dvy, T &dvz,
                                          const T &gamma, const T &gamma1,
